...
int main() {
	...
	for(i = 0; i < 3; i++) {
		// 1. Imprimimos los datos en columnas fijas
		printf("%-12d ", clientes[i].cliente_nro);
		
		if(clientes[i].cliente_tipo == 'E') {
			printf("%-14s ", "Empresarial");
			total_tipo_E++; // Sumamos al contador de Empresas
		} else {
			printf("%-14s ", "Privado");
			total_tipo_P++; // Sumamos al contador de Privados
		}
		
		printf("%-16s ", clientes[i].cliente_nombre);
		printf("%s\n", clientes[i].cliente_cel);
		
		total_clientes++; // Sumamos uno al total general
	}
	
	printf("-------------------------------------------------------------\n");
	// Mostramos los totales calculados de forma automática
